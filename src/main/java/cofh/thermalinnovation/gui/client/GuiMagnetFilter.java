...
public class GuiMagnetFilter extends GuiContainerCore {
	...
	public GuiMagnetFilter(InventoryPlayer inventory, ContainerMagnetFilter container) {
		...
		level = ItemMagnet.getLevel(container.getFilterStack());
		secure = SecurityHelper.isSecure(container.getFilterStack());

		playerName = SecurityHelper.getID(inventory.player);
		filterIndex = ItemMagnet.getLevel(container.getFilterStack());
	...
	public void handleElementButtonClick(String buttonName, int mouseButton) {
		...
		ContainerMagnetFilter container = (ContainerMagnetFilter) inventorySlots;
		int flag = 0;
		switch (buttonName) {
			case "FilterList":
